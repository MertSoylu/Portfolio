    ...
    >
      ...
      >
        ...
        {index != null && (
          <span
            aria-hidden="true"
            className="pointer-events-none absolute -bottom-7 -right-1 select-none text-[7rem] font-extrabold leading-none text-ink-900/[0.05] transition-colors duration-500 group-hover:text-violet-500/[0.08] dark:text-white/[0.05] dark:group-hover:text-aqua-200/[0.08]"
            style={{ fontFamily: "'Syne', 'Bricolage Grotesque', system-ui, sans-serif" }}
          >
            {String(index).padStart(2, '0')}
          </span>
        )}
        <div className="relative z-10 flex h-full flex-col [transform:translateZ(26px)]">{children}</div>
      </motion.div>
    </motion.div>
  );
};
